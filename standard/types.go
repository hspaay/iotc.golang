...
func NewInput(node *Node, ioType string, instance string) *InOutput {
	address := MakeInputDiscoveryAddress(node.Zone, node.PublisherID, node.ID, ioType, instance)
	io := &InOutput{
		Address:  address,
...
// NewConfig instance for holding node configuration
func NewConfig(id string, dataType DataType, description string, value string) *ConfigAttr {
	config := ConfigAttr{
		ID:          id,
		DataType:    dataType,
