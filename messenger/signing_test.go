...
func TestEcdsaSigning(t *testing.T) {
	...
	sig1 := CreateEcdsaSignature(string(payload1), privKey)
	match := VerifyEcdsaSignature(string(payload1), sig1, &privKey.PublicKey)
...
func TestJWSSigning(t *testing.T) {
	...
	sig1, err := CreateJWSSignature(string(payload1), privKey)
	assert.NoErrorf(t, err, "signing node1 failed")
	assert.NotEmpty(t, sig1, "Signature is empty")

	sig2 := CreateEcdsaSignature(string(payload1), privKey)
...
func TestSigningPerformance(t *testing.T) {
	...
	for count := 0; count < 10000; count++ {
		...
		sig := CreateEcdsaSignature(payload1base64, privKey)
		_ = sig
	}
	...
	start = time.Now()
	for count := 0; count < 10000; count++ {
		CreateJWSSignature(string(payload1), privKey)
	}
	duration = time.Since(start).Seconds()
	...
	for count := 0; count < 10000; count++ {
		...
		match := VerifyEcdsaSignature(payload1base64, sig, &privKey.PublicKey)
		_ = match
	}
	duration = time.Since(start).Seconds()
	...
	start = time.Now()
	for count := 0; count < 10000; count++ {
		VerifyJWSMessage(sig1, &privKey.PublicKey)
	}
	duration = time.Since(start).Seconds()
...
func TestVerifyPublisher(t *testing.T) {
	...
	isSigned, err := VerifySender(sig1, &received, func(address string) *ecdsa.PublicKey {
