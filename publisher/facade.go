...
func (publisher *Publisher) GetNodeByID(nodeID string) (node *iotc.NodeDiscoveryMessage) {
	node = publisher.Nodes.GetNodeByID(publisher.zone, publisher.publisherID, nodeID)
	return node
}
...
func (publisher *Publisher) GetOutputByType(nodeID string, outputType string, instance string) *iotc.OutputDiscoveryMessage {
	nodeAddr := nodes.MakeNodeDiscoveryAddress(publisher.zone, publisher.publisherID, nodeID)
...
// MakeNodeDiscoveryAddress makes the node discovery address using the publisher zone and publisherID
func (publisher *Publisher) MakeNodeDiscoveryAddress(nodeID string) string {
	addr := nodes.MakeNodeDiscoveryAddress(publisher.zone, publisher.publisherID, nodeID)
	return addr
}

// NewNode creates a new node and add it to this publisher's discovered nodes
// This is a convenience function that uses the publisher zone and id to create a node in its node list.
// returns the node's address
func (publisher *Publisher) NewNode(nodeID string, nodeType iotc.NodeType) string {
	addr := publisher.Nodes.NewNode(publisher.zone, publisher.publisherID, nodeID, nodeType)
	return addr
}
